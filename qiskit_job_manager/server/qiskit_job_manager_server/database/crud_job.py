...
# Retrieve a job with a matching IBM job ID
async def retrieve_job_by_ibm_job_id(ibm_job_id: str, user: User) -> dict:
    if user.is_superuser:
        job = await job_collection.find_one({"job_id": ibm_job_id})
    else:
        job = await job_collection.find_one({"job_id": ibm_job_id, "owner_id": str(user.id)})
    if job:
        return job_helper(job)
...
# Update a job with a matching IBM job ID
async def update_job_by_ibm_job_id(ibm_job_id: str, data: dict, user: User):
    # Return false if an empty request body is sent.
...
# Delete a job from the database
async def delete_job_by_ibm_job_id(ibm_job_id: str, user: User):
    if user.is_superuser:
        job = await job_collection.find_one({"job_id": ibm_job_id})
    else:
        job = await job_collection.find_one({"job_id": ibm_job_id, "owner_id": str(user.id)})
    if job:
        await job_collection.delete_one({"job_id": ibm_job_id})
        return True
